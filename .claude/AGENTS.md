...
## 2. Workflow: plan → delegate → implement → review

**Permissions:** full autonomy ("allow all") for implementation work — editing files,
running builds/tests/linters, installing deps, local commits. The hard boundary is the
remote: nothing leaves the machine without me (see git policy below).

- **Always plan first.** Before implementation, produce a short plan (steps, files
  touched, risks). Execution follows the accepted plan; deviations get flagged, not
  silently improvised.
- **Always split work across agents** where tasks are independent — parallelize
  investigation and implementation, reconcile before reporting.
- **Match the agent's model to the task.** Strong model for architecture, tricky
  debugging, and code review; cheaper/faster model for mechanical work (renames,
  boilerplate, running gates).
- **After implementation, always:**
  1. Delegate code review to a separate agent (fresh context, strong model).
  2. Run the full test suite and gates; confirm no regressions.
  3. Report a short summary: what changed, current status, what we gained — terse.
  4. List the commit batches (names) and provide the ready-to-run PR command with
     title + description. I push manually, or the agent pushes only on my explicit order.

### Git & PR policy

- **Never push. Never create a PR without confirmation.** Commits stay local; hand me the
  push/PR command instead.
- **Never credit AI in commits or PRs.** No `Co-Authored-By`, no "Generated with…"
  trailers, no AI mentions in messages or descriptions.
- **Conventional commits, always** (`feat:`, `fix:`, `chore:`, `refactor:`, `test:`, …).
- **Branch names carry an incrementing number**, e.g. `feat/042-import-retry` — next
...
## 6. Tests confirm the solution — "it compiles" is not done
